...
export const adminDashBoard = async(_req: Request, res: Response) => {
    ...
    return res.json(serializeResponse('success', { users: userCount }, "Users data"));
}
...
export const getRiderPerformanceReport = async (req: Request, res: Response, next: NextFunction) => {
  const parsedQuery = riderPerformanceSchema.safeParse(req.query);

  if (!parsedQuery.success) {
    const issues = parsedQuery.error.issues.map(issue => issue.message).join(', ');
  ...
  try {
    ...
    const riderPerformance = await OrderModel.aggregate([
      { $match: query },
      {
        $group: {
          _id: '$riderId',
          totalDeliveries: { $sum: { $cond: [{ $eq: ['$orderStatus', 'delivered'] }, 1, 0] } },
          totalCancellations: { $sum: { $cond: [{ $eq: ['$orderStatus', 'cancelled'] }, 1, 0] } },
          totalDeliveryTime: {
            $sum: {
              $cond: {
      ...
      },
      {
        $project: {
          ...
          averageDeliveryTimeMinutes: {
            $divide: [{ $divide: ['$totalDeliveryTime', 60000] }, '$totalDeliveries']
          },
        }
      },
    ]);
    ...
    if (!riderPerformance || riderPerformance.length === 0) {
      return res.status(200).json(serializeResponse('success', { riderPerformance: [] }, 'No performance data found for the given criteria.'));
    }

    return res.status(200).json(serializeResponse('success', { riderPerformance }, 'Rider performance report fetched successfully.'));
...
export const deactivateUser = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    return res.status(200).json(serializeResponse('success', { user }, 'User account deactivated successfully.'));
  } catch (error) {
    next(error);
  }
};
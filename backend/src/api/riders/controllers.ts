  ...
  return res
    ...
    .json(
      serializeResponse(
        "success",
        { history },
        "Rider order history retrieved successfully."
      )
    );
};

export const viewOrderDetails = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  return res
    ...
    .json(
      serializeResponse(
        "success",
        { orderDetails },
        "Details of order retrieved successfully."
      )
    );
};
...
export const acceptOrderAssignment = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  try {
    ...
    if (!order) {
      const err = new Error(
        "Order not found or not available for assignment."
      ) as any;
      err.statusCode = 404;
      return next(err);
    }
    ...
    return res
      ...
      .json(
        serializeResponse(
          "success",
          { order },
          "Order assignment accepted successfully."
        )
      );
  } catch (error) {
    next(error);

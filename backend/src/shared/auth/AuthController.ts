...
export const register = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!authBodyParser.success) {
        const issues = authBodyParser.error.issues.map(issue => issue.message).join(', ');
...
export const login = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!loginBodyParser.success) {
        const issues = loginBodyParser.error.issues.map(issue => issue.message).join(', ');
        const err = new Error(`Invalid request body: ${issues}`) as any;
        err.statusCode = 400;
        return next(err);
    }
    ...
    if (!user.validated) {
        const err = new Error('Account not validated. Please verify your email.') as any;
        err.statusCode = 403;
        return next(err);
    }
...
export const refreshToken = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        if (error.name === 'TokenExpiredError' || error.name === 'JsonWebTokenError') {
            ...
            const err = new Error('Refresh token expired or invalid. Please log in again.') as any;
            err.statusCode = 401;
            return next(err);
        }
        return next(error);
...
export const verifyUserAccount = async (req: Request, res: Response, next: NextFunction) => {
    const verifyUserAccountBodyParser = verifyUserAccountSchema.safeParse(req.body);
    if (!verifyUserAccountBodyParser.success) {
        const issues = verifyUserAccountBodyParser.error.issues.map(issue => issue.message).join(', ');
        const err = new Error(`Invalid request body: ${issues}`) as any;
        err.statusCode = 400;
        return next(err);
    }
    ...
    if (user.validationCodeExpireDate && user.validationCodeExpireDate < new Date()) {
    ...
    user.validated = true;
    await user.save();
    return res.status(201).json(serializeResponse('success', null, 'Account validated successfully. You can log in. '));
};
...
export const forgotPassword = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!forgetPasswordBodyParser.success) {
        const issues = forgetPasswordBodyParser.error.issues.map(issue => issue.message).join(', ');
        const err = new Error(`Invalid request body: ${issues}`) as any;
        err.statusCode = 400;
        return next(err);
    }
    ...
    if (!user) {
        return res.status(200).json(serializeResponse('success', null, 'If a user exists, a password reset link has been sent to their email.'));
    }
    ...
    await user.save();
    await sendMail(email, 'Reset your password', `Reset password code: ${tokenDigits}`).catch(console.error);
    
    return res.status(200).json(serializeResponse('success', null, 'If a user exists, a reset link was sent.'));
};
...
export const resetPassword = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!resetPasswordBodyParser.success) {
        const issues = resetPasswordBodyParser.error.issues.map(issue => issue.message).join(', ');
        const err = new Error(`Invalid request body: ${issues}`) as any;
        err.statusCode = 400;
        return next(err);
    }
...
export const resendVerificationCode = async (req: Request, res: Response, next: NextFunction) => {
    const resendVerificationCodeBodyParser = resendVerificationCodeSchema.safeParse(req.body);
    if (!resendVerificationCodeBodyParser.success) {
        const issues = resendVerificationCodeBodyParser.error.issues.map(issue => issue.message).join(', ');
        const err = new Error(`Invalid request body: ${issues}`) as any;
        err.statusCode = 400;
        return next(err);
    }
    const { email } = resendVerificationCodeBodyParser.data;
    const user = await User.findOne({ email }).select('+validateCode +validationCodeExpireDate');
    if (!user) {
        const err = new Error('User not found.') as any;
        err.statusCode = 404;
        return next(err);
    ...
    await user.save();
    await sendMail(user.email, 'New Verification Code', `Your new verification code is: ${newCode}`).catch(console.error);
    
    return res.status(200).json(serializeResponse('success', null, 'A new verification code has been sent to your email.'));
};